...
def score_timing(idx, px, big7, big7_wma, sell, buy, buy_zone, crash_zone, df, window: int = 50):
    # --- Breadth counts ---
    cnt_below = pd.DataFrame({
        t: (px[t] < weighted_ma(px[t], n=window)).astype(int) for t in big7
    }).sum(axis=1)
    cnt_above = pd.DataFrame({
        t: (px[t] > weighted_ma(px[t], n=window)).astype(int) for t in big7
    }).sum(axis=1)
    ...
    # --- Choice tables ---
    buy_zone_choices     = [-1.2, -0.7]  # strong buy zone
    not_buy_zone_choices = [-2.0, -1.4]  # mild buy zone

    normal_sell_choices  = [1.0, 0.6]
    crash_sell_choices   = [2.2, 1.6]

    # --- Breadth persistence ---
    weak_breadth = (df["cnt_below"] >= 4).rolling(2, min_periods=2).sum() == 2

    # --- Sell conditions ---
    sb1 = sell & weak_breadth & (df["big7_px"] < df["big7_wma"])
    sb2 = sell & (df["big7_px"] * 1.05 < df["big7_wma"])  # deeper drop

    # --- Buy persistence state ---
    state   = False
    states  = []
    ...
    bf2roll = bf2.rolling(2, min_periods=2).sum() > 1

    # --- Buy / sell condition lists ---
